...
- current production genplan status on 2026-08-17: all 641 manual candidate points reviewed (`queued=0`); 426 `urban_plan_layers`, including 111 active `VERIFIED_STRICT/search` rows in 37 scopes, 99 inactive `SUPERSEDED` rows and 216 inactive `REVIEWED_HOLD` rows. The former 336-row shadow backlog is fully resolved and contains no unreviewed `WARNING`. Active rows: `LPH` 3, `LPH:household` 84, `LPH:field` 3, `Gardening` 21. Canonical status: `docs/GENPLAN_STATUS_2026_08_17.md`;
