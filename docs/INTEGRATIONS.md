...
- Для заблокированных результатов веб-поиска применяется direct-link сценарий: активный `payment_provider_url` ApiPay (`qr.kaspi.kz`) передается в карточку как обычная ссылка `Оплатить в Kaspi`; мобильный flow не зависит от QR, модалки или `fetch`. Полное правило и диагностика: `docs/MOBILE_KASPI_PAYMENT.md`.
