...
Production-снимок на 17.08.2026 после итогового разбора: 426 строк
`urban_plan_layers`, из них 111 активных `VERIFIED_STRICT/search` в 37
территориально-целевых областях применения, 99 `SUPERSEDED` и 216
`REVIEWED_HOLD`. Необработанных `WARNING` нет. Ручная очередь рассчитанных точек
